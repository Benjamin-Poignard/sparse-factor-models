...
% set h = 24 for 24 forecast horizon
...
for i=1:TT-T-h+1
    ...
    for j = 1:K
        ...
        % Sparse Approximate Factor model
        gamma = (0.001:0.01:10)*sqrt(log(p*(p+1)/2)/T);
        ...
        % Sparse Factor model, Gaussian - MCP
        [Lambda_first,Psi_first] = non_penalized_factor(Y',r,'Gaussian');
        ...
        % Sparse Factor model, LS - SCAD
        [Lambda_first,Psi_first] = non_penalized_factor(Y',r,'LS');
        loss = 'LS'; method = 'scad';
        gamma = (0.001:0.05:2)*sqrt(log(p*r)/T);
