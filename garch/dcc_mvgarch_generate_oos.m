function [logL,Rt,likelihoods,Qt]=dcc_mvgarch_generate_oos(parameter,data_out,data_in,h_in)

% Out-of-sample full log-likelihood of the scalar DCC process with 
% parameters estimated in-sample

% Inputs:
%        - parameter: 3*p + 2 x 1 parameter vector of interest (includes
%        the univariate GARCH(1,1) parameters and scalar DCC parameters)
%        - data_out: T_out x p matrix of obversations, out-of-sample, with
%        T_out the length of the out-of-sample period
%        - data_in: T_in x p matrix of observations, in-sample, with T_in
%        the length of the in-sample period
%        - h_in: T_in x p vector of univariate conditional variance
%        processes, in-sample

% Outputs:
%        - logL: composite log-likelihood value evaluated at parameter
%        -  Rt: p x p x T_out correlation process generated from the scalar
%        DCC
%        - likelihoods: T_out x 1 vector of log-likelihood evaluated at
%        parameter such that logL = sum(likelihoods)
%        - Qt: p x p x T_out process of the underlying Qt matrix generated 
